...
ORG !free89	;!Freespace_HexMapSelectSwitch
HexMapMenuMain:
	PHB
...
;	LDA #!Freespace_HexMapSelectSwitch>>$10 : STA $14 : JML [$0012]
	LDA #HexMapMenuMain>>$10 : STA $14 : JML [$0012]
...
--- : PHX : CPX $079F : BEQ +		;check index is current area
...
HexMapPrepareZoomOut:	;pull bank $7E
	IF !DynamicHexMapData
		LDA $079F : ASL : CLC : ADC $0A : TAX : LDA.l $810000,x
		SEC : SBC #$000C : STA $4050	;zoom transition timer
		LDA $079F : ASL #4 : CLC : ADC $08 : TAX
	...
	ELSE
		LDA $079F : ASL : TAX : LDA.l $81AA94,x
		SEC : SBC #$000C : STA $4050	;zoom transition timer
		TXA : ASL #3 : TAX
	;Square transition speed
...
--- : LSR $1A : BCC ++											;check if area explored
	LDX #$0200 : LDA $1C : CMP $079F : BNE + : LDX #$0000		;set palette based on selected area
+ : STX $03 : ASL #2
	IF !DynamicHexMapData
...
;Check if new area selected
	...
	LDX $079F : JSR HexMapGrayedAreaPalette					;pull bank has to be $81
...
--- : ASL $1A : BCC +++						;check if area is explored
	LDA $1C : CMP $079F : BEQ +++ : ASL #2	;cancel if in current area
	IF !DynamicHexMapData
		CLC : ADC $06 : TAX
...
HexMapPrepareZoomIn:	;pull bank $7E
	IF !DynamicHexMapData
		LDA $079F : ASL : CLC : ADC $0A : TAX : LDA.l $810000,x : STA $4050	;zoom transition timer
	;Square transition starting position
		LDA $079F : ASL #2 : CLC : ADC $06 : TAX
	...
	;Square transition speed
		...
		LDA.l $81000C,x : STA $404C : LDA.l $81000E,x : STA $404E	;bottom
	ELSE
		LDA $079F : ASL : TAX : LDA.l $81AA94,x : STA $4050
	;Square transition starting position
		TXA : ASL : TAX
...
;Load new area label
	...
	IF !DynamicHexMapData
		PHX : LDA $079F : ASL : CLC : ADC.l $829429 : TAX : LDA.l $820000,x : PLX
	ELSE
		PHX : LDA $079F : ASL : TAX : LDA.l $82965F,x : PLX
	ENDIF
...
PostHexMap_MapConstruction:	;pull bank $82
	...
	LDX #$005E
	LDX $079F : LDA $7ED908,x : AND #$00FF : STA $0789	;set flag of map station for next area
