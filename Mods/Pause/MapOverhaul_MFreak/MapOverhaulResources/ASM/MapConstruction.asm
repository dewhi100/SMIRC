...
; $04 = Map (destination)
MapDecoration:
	PHB : PEA.w MapDecoration_AreaPointer>>8 : PLB : PLB		;set bank to decoration data
...
;set background data
	SEP #$30
	LDX $079F : LDA.l VerticalAreaMapBit : AND $82B7C9,x : BEQ +	;check for vertical bit of current area
...
;check for any doorbits who add sprites on the map
	...
	LDA $079F : ASL : TAX : LDA.w DoorSpriteAreaCount,x : DEC : BMI ++ : STA $00
	LDA.w DoorSpriteAreaList,x : STA $06 : TAX
-- : LDA.l DoorSprite_Lists&$FF0000,x : AND #$01FF : JSL $80818E	;get door bit index of current entry
	LDA $7ED8B0,x : AND $05E7 : BNE +		;check if door bit is not set
	LDX $06 : LDA.l (DoorSprite_Lists&$FF0000)+$01,x : LSR
...
AddSpriteWithDoorbit:
	...
	LDA $079F : CMP !OriginAreaIndex : BNE + : LDA $07F7,y : BRA ++		;load explored bit of current area if in current area
...
++ : SEP #$20 : BIT $B88A,x : BEQ +++		;check if explored bit is set
	LDX $079F : LDA.l VerticalAreaMapBit : BIT $B7C9,x : REP #$20 : BEQ +	;check if vertical area is set
...
MapTransitionBG1Branch:
	...
	LDX $079F : LDA.l VerticalAreaMapBit : AND $82B7C9,x : BEQ +
