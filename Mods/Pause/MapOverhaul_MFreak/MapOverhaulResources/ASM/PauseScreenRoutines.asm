...
++ : PHB : PEA.w Maptile_Glows>>8 : PLB : PLB			;set bank to where maptile glow data is
...
if !PercentTime_FelicityVi != 0
	...
	BEQ ++				;check if reserve activated (in auto/manual mode)
endif	
	LDX #$0082 : STX $02

;Integrate with Tundain's custom Equip Screen
;if !EquipScreen_Tundain == 1
;	LDX #Auto : CMP #$0001 : BEQ +	;AUTO tile offset
;	LDX #Manual : + : STX $00		;load MANUAL offset instead if in manual mode
;else
