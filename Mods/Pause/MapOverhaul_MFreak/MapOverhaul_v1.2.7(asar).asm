...
;	!Freespace_MapConstruction = $85C000 ;replaced with !free85

;Location for maptile GFX for minimap (size: $1000)
	!Freespace_MinimapTiles = $9AC200	;Despite the name, this is not normal freespace

;Location of code to transfer tile graphics from RAM to VRAM (restricted to bank $80) (size: $97)
;	!FreespaceBank80_VRAM = $80CD8E	;vanilla freespace. ends at FC00 instead of end of bank like modst banks do. ;Unused, replaced with !free80
...
;	!Freespace_CoverTiles = $B6F200	;vanilla freespace start	;replaced with !freeB6
}
...
;Location of map decoration data (original size: $C8)
	!Freespace_MapDecoration = $89E000	;	well into freespace. at a guess I would think this goes exactly to the bank boundary
...
;	!Freespace_MaptileGlow = $8EE600	;beginning of vanilla freespace
...
;	!Freespace_HexMapSelectSwitch = $89C000	;well into freespace. again, using end of freespace rather than beginning?
...
;Bin file
;ORG !Freespace_MinimapTiles	;not needed SMART already has gfx.
;Freespace_MinimapTiles: 
;INCBIN MapTiles.bin	;maptile storage for HUD map

}

;Normally, you still need to apply the graphics IPS *even if you have the SMART XML all set up*
;This is because the patch also includes tilemapping for the "select" and "start" buttons at the bottom center of the pause screen
;Without it, you see ugly red X tiles. This is meant to fix that and remove the need to reapply the IPS every time you refresh the ROM
incsrc "MapOverhaulButtons.asm"