...
!HAMMERBALL_COST = $0014
!HAMMERBALL_COST_CONTINUOUS = $0400
;-------------Constants---------------------------------------------------------| 
;Define				 ; Description    											|
;-------------------------------------------------------------------------------|
...
; JMP EndHMB		;if suitless, and not in hammerball state, disable HB (original code, doesnt properly handle entry into water from air.
...
if !HammerBallTriggerWithJump == 0
	...
	;ONE-TIME SUITLESS CHECK
;	LDA $0AD2 : BEQ ++ : LDA $09A2 : BIT #$0020 : BNE ++
;	LDA #!HAMMERBALL_COST : JSL SpendAir : BCC +
;	++
